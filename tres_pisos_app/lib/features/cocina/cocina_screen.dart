          ...
          Padding(
            ...
            child: Row(children: [
              Container(
                ...
                decoration: BoxDecoration(color: AppColors.amber.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8)),
                child: Text('${pedidos.length} activos', style: const TextStyle(color: AppColors.amber, fontWeight: FontWeight.w600, fontSize: 12)),
              ),
      ...
      body: loading
        ...
        : pedidos.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.check_circle_outline, size: 60, color: AppColors.success.withValues(alpha: 0.4)),
              const SizedBox(height: 16),
              const Text('Sin pedidos pendientes', style: TextStyle(color: AppColors.muted, fontSize: 15)),
            ]))
          : RefreshIndicator(
...
class _PedidoCard extends StatelessWidget {
  ...
  const _PedidoCard({required this.pedido, required this.tiempo, required this.onCambiar});

  bool get _urgente => DateTime.now().difference(pedido.creadoEn).inMinutes >= 15 && pedido.isPendiente;
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _urgente ? AppColors.danger : AppColors.border, width: _urgente ? 1.5 : 1),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Container(
          ...
          child: Row(children: [
            Expanded(child: Text('Mesa ${pedido.mesa}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15))),
        ...
        Padding(
          ...
          child: Row(children: [
            Icon(Icons.schedule, size: 13, color: _urgente ? AppColors.danger : AppColors.muted),
            const SizedBox(width: 4),
            Text(tiempo, style: TextStyle(fontSize: 12, color: _urgente ? AppColors.danger : AppColors.muted)),
            if (_urgente) ...[const SizedBox(width: 6), const Icon(Icons.warning_amber_rounded, size: 14, color: AppColors.danger)],
          ]),
        ),
        // Productos
        Expanded(
          child: ListView(padding: const EdgeInsets.fromLTRB(12, 8, 12, 0), children: [
            ...pedido.productos.map((p) => Padding(
              padding: const EdgeInsets.only(bottom: 5),
              child: Row(children: [
                Container(
                  width: 22, height: 22,
                  decoration: BoxDecoration(color: AppColors.amber.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(5)),
                  child: Center(child: Text('${p.cantidad}', style: const TextStyle(color: AppColors.amber, fontWeight: FontWeight.w700, fontSize: 11))),
                ),
                const SizedBox(width: 8),
                Expanded(child: Text(p.nombre, style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis)),
              ]),
            )),
          ]),
        ),
